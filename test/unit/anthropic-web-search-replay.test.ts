...
describe("Anthropic Web Search history replay", () => {
  ...
  it("allows typed callers to replay gateway responses directly", () => {
    ...
    const request: AnthropicMessageRequest = {
      model: "test-model",
      max_tokens: 128,
      messages: [response, { role: "user", content: "continue" }],
    };
    expect(decodeAnthropicRequest(request).messages).toEqual([
      { role: "assistant", content: [{ type: "text", text: "answer" }] },
      { role: "user", content: [{ type: "text", text: "continue" }] },
    ]);
  });
