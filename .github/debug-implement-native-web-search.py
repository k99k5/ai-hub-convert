...
# web-search-loop.ts: carry real provider execution metadata through synthesized streams.
replace_once(
    "src/upstream/web-search-loop.ts",
    '''
...
# client.ts: retain the real query and provider results for every search execution.
replace_once(
...
# app.ts: decode trace metadata before the synthesized stream body is consumed.
replace_once(
    "src/app.ts",
    '''
...
# stream-encode.ts: replace the count-only probe with actual native search blocks.
replace_once(
...
p = Path("src/protocols/anthropic/stream-encode.ts")
text = p.read_text()
start_marker = '  #complete(event: Extract<CanonicalEvent, { type: "response_complete" }>): AnthropicSseFrame[] {'
end_marker = "\n  #assertStarted(): void {"
...
replacement = d('''
  #complete(event: Extract<CanonicalEvent, { type: "response_complete" }>): AnthropicSseFrame[] {
    ...
    let nextIndex =
      this.#seenIndices.size === 0 ? 0 : Math.max(...this.#seenIndices.values()) + 1;
...
# Tests: trace propagation and native payload correctness.
replace_once(
...
test_path = Path("test/unit/web-search-usage.test.ts")
...
test_text = test_path.read_text()
ending = "\n});\n"
if not test_text.endswith(ending):
    raise RuntimeError("unexpected test file ending")
native_test = d('''
...
''')
test_path.write_text(test_text[:-len(ending)] + "\n" + native_test + ending)

# The workflow and this script are one-shot helpers and should not remain on the branch.
Path(".github/workflows/debug-implement-native-web-search.yml").unlink(missing_ok=True)
Path(".github/debug-implement-native-web-search.py").unlink(missing_ok=True)
