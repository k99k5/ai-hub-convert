...
11. `conversation` 使用独立的有界内存存储，提供创建、读取、更新、删除及历史项管理 API。按凭据 HMAC 隔离，HTTP/WS 共用，不绑定连接或模型；每轮只在完整成功后原子追加 input/output。上游只接收展开后的输入及 `store:false`，不接收本地 conversation ID。同一会话执行期间其他修改返回 409，容量不足明确报错，不自动淘汰旧会话。会话保留至删除或进程重启，不使用数据库或持久化存储。
