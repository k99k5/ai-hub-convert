...
已接受；保留 Responses 引用和 HTTP 增量续轮所需的有界短期内存状态，以及 WS 连接内的有界历史。2026-09-18 增补 WebSocket 传输和 HTTP previous_response_id 续轮。
...
目标上游不能保证解析 `item_reference`。单纯接受并透传该字段不能完成 Chatbox 续轮，删除未命中引用又会丢失历史。采用输出项缓存展开引用，并用独立的有界短期完整历史支持 HTTP `previous_response_id`；不建设持久化会话存储。WS 续轮仅在存活连接内展开 ID，不提供跨连接恢复。HTTP 增量续轮预算和错误契约见[HTTP 历史缓存](../compatibility.md#responses-http-历史缓存)。
