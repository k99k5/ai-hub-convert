...
Responses → Responses 接受显式 `{type:"item_reference", id:"非空字符串"}`，仅允许这两个字段，按原顺序透传给上游。引用在 canonical 请求中作为独立输入项保存，不改写为消息正文，不跨协议转换，也不进入计数路径。网关默认 `store:false`；调用方显式 `store:true | false | null` 保持原值，不因出现引用而自动开启存储。引用能否解析取决于上游是否支持引用以及对应对象是否仍可访问；上游错误沿用现有清洗规则，不回退或重试。网关不保存或还原被引用的历史内容。
